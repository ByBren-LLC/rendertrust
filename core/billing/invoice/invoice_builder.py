...
async def build(account_id:str):
    async with async_session() as s:
        rows=await s.execute("""SELECT created_at, delta_usd FROM ledger_entries WHERE account_id=:a AND date_trunc('month', created_at)=date_trunc('month', now()-interval '1 month')""",{"a":account_id})
        items=[{"date":r.created_at.date(),"desc":"Ledger activity","amount":r.delta_usd} for r in rows]
        total=sum(i['amount'] for i in items)
    html=TEMPL.render(account={"id":account_id,"name":account_id},items=items,total=total,period=PERIOD)
    ...
    url=S3.generate_presigned_url('get_object',Params={'Bucket':'wtfb-invoices','Key':key},ExpiresIn=2592000)
    return url,total
